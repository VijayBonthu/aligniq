...
export default function ChatView() {
  ...
  // Inline-styled header can't be media-queried, so branch on viewport: a compact
  // header + overflow menu on tablet/phone, and drop the 252px integrations rail
  // on small screens so the chat gets the full width.
  const compact = useMediaQuery('(max-width: 1024px)');
  const [menuOpen, setMenuOpen] = useState(false);
  // Integrations panel: inline + collapsible on desktop, off-canvas drawer on
  // tablet/phone. Open by default on desktop, closed on small screens.
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              height: 28,
              borderRadius: 999,
              border: `1px solid ${integrationsOpen ? 'var(--accent)' : 'var(--border)'}`,
              background: integrationsOpen ? 'var(--accent-soft)' : 'transparent',
              color: integrationsOpen ? 'var(--accent)' : 'var(--fg-muted)',
              cursor: 'pointer',
            }}
          >
        ...
        {compact && (
          <div style={{ position: 'relative', flexShrink: 0, zIndex: 10 }}>
            ...
            >
              <button onClick={() => navigate('/projects')} aria-label="Projects" style={iconBtn}>
              ...
              <span title="Full report ready" style={{ width: 7, height: 7, borderRadius: '50%', background: 'var(--ok)', flexShrink: 0 }} />
              <button onClick={() => setMenuOpen((o) => !o)} aria-label="Menu" style={iconBtn}>
            ...
            {menuOpen && (
              <>
                ...
                >
                  <p style={menuLabel}>VIEW</p>
                  <MenuItem active={tab === 'chat'} onClick={() => { setTab('chat'); setMenuOpen(false); }}>
                    Chat
                  </MenuItem>
                  <MenuItem
                    active={tab === 'premortem'}
                    disabled={!reportReady}
                    onClick={() => { if (reportReady) { setTab('premortem'); setMenuOpen(false); } }}
                  >
                    Pre-Mortem
                  </MenuItem>
                  ...
                  <MenuItem
                    disabled={!reportReady}
                    onClick={() => { if (reportReady) navigate(`/deliverable/${chatHistoryId}`); }}
                  >
                    Build Deliverable
                  </MenuItem>
                  <MenuItem
                    disabled={!reportReady}
                    onClick={() => { if (reportReady) { setChangeDraft(undefined); setPendingOpen(true); setMenuOpen(false); } }}
                  >
                    Changes
                  </MenuItem>
                  <MenuItem
                  ...
                  <MenuItem
                    active={contextMode}
                    disabled={tab !== 'chat'}
                    onClick={() => { if (tab === 'chat') setContextMode((p) => !p); }}
                  >
                    {contextMode ? `Context · ${selectedCount}/${messages.length}` : 'Select context'}
                  </MenuItem>
                  <div style={menuDivider} />
                  <MenuItem onClick={() => { setIntegrationsOpen(true); setMenuOpen(false); }}>
                    Integrations
                  </MenuItem>
                </div>
              </>
        ...
        {contextMode && (
          <div
            style={{
              ...
              borderBottom: '1px solid rgba(52,163,123,.2)',
              display: 'flex',
              alignItems: 'center',
          ...
          <div style={{ maxWidth: 780, margin: '0 auto', padding: '0 clamp(14px, 4vw, 28px)' }}>
            {messages.length === 0 && (
              ...
              >
                ...
                >
                  Ask about risks, architecture, timeline, or cost — or push items
                  straight to your connected tools.
                </p>
              </div>
            )}
            {savedOnly && (
        ...
        <div
          style={{
            ...
            background: 'var(--surface)',
            padding: '10px clamp(12px, 4vw, 18px) 14px',
          }}
        >
              ...
              {isStreaming ? (
                ...
                <button
                  ...
                  style={{
                    ...
                    color: atCap ? 'var(--fg-muted)' : input.trim() ? 'var(--accent-ink)' : 'var(--fg-muted)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
...
const menuDivider: React.CSSProperties = { height: 1, background: 'var(--border)', margin: '4px 6px' };
  ...
  return (
    <button
      ...
      style={{
        ...
        color: disabled ? 'var(--fg-muted)' : active ? 'var(--accent)' : 'var(--fg-dim)',
        fontSize: 13,
