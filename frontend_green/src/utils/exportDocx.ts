...
/** Build the deliverable as a .docx Blob (no download). Shared by the download button
 * and the "Send to Jira" attach flow. `opts.node` is the rendered preview, used to
 * rasterize mermaid diagrams into the document. */
...
export async function exportMarkdownToDocx(
  ...
  const blob = await buildDocxBlob(markdown, opts);
