...
const ACCENT = '#1f7d5b'; // GroundedIQ print emerald (light-mode accent, AA on parchment)
const FG = '#1c1c1c';
const FG_DIM = '#3a3a3a';
...
function coverPage(title: string): PdfContent {
  ...
  return {
    stack: [
      { text: 'GROUNDEDIQ', color: ACCENT, bold: true, fontSize: 12, margin: [0, 120, 0, 0] },
      ...
      { text: 'Generated by the GroundedIQ multi-agent analysis pipeline.', color: FG_MUTED, fontSize: 11 },
...
/** Build the pdfmake instance + document definition (fonts, cover, body, diagrams).
 * Shared by the download path and the Blob/Send-to-Jira path. */
async function preparePdf(
  markdown: string,
  opts: { title?: string; node?: HTMLElement | null } = {},
): Promise<{ pdfMake: any; docDefinition: PdfContent }> {
  ...
  const docDefinition: PdfContent = {
    ...
    footer: (currentPage: number, pageCount: number): PdfContent => {
      ...
      return {
        margin: [48, 12, 48, 0],
        columns: [
          { text: 'GroundedIQ · Generated Report', fontSize: 8, color: FG_MUTED },
    ...
    header: (currentPage: number): PdfContent => {
      ...
      return {
        margin: [48, 18, 48, 0],
        columns: [
          { text: 'GROUNDEDIQ', fontSize: 8, bold: true, color: ACCENT },
...
/** Build the deliverable as a PDF Blob (no download) — for the "Send to Jira" attach flow. */
export async function buildPdfBlob(
  markdown: string,
  opts: { title?: string; node?: HTMLElement | null } = {},
): Promise<Blob> {
  const { pdfMake, docDefinition } = await preparePdf(markdown, opts);
  // pdfmake 0.3.x: getBlob() returns a Promise<Blob> (and does NOT call a callback).
  // 0.2.x: getBlob(cb) invokes the callback and returns undefined. Support both so the
  // call can't hang on the version mismatch.
  const ret = pdfMake.createPdf(docDefinition).getBlob();
  if (ret && typeof ret.then === 'function') return ret as Promise<Blob>;
  return new Promise<Blob>((resolve) => pdfMake.createPdf(docDefinition).getBlob((b: Blob) => resolve(b)));
}
